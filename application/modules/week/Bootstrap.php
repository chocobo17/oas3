...
class Week_Bootstrap extends Zend_Application_Module_Bootstrap
{
	...
	protected function _initSessionConst()
	{
		...
		$data = $this->_config->fetchAll("configType = 'WEEK'")->toArray();
		if( !is_array($this->_session->CONST->WEEK) ){
			if( count($data) == 0){
				//先寫入初始化設定值部份！
				$this->_initConfigure();
				$data = $this->_config->fetchAll("configType = 'WEEK'")->toArray();
			...
			foreach($data as $key => $value){
				$this->_session->CONST->WEEK[$value['configkey']]=$value['value'];
			}
		}elseif(count($data) > 0){
			foreach($data as $key => $value){
				if( $this->_session->CONST->WEEK[$value['configkey']] !=$value['value']){
					$this->_session->CONST->WEEK[$value['configkey']]=$value['value'];
				}
			}
		}					
	...
	protected function _initConfigure()
	{
		...
		if( !file_exists(realpath(dirname(__FILE__)).'/configure.xml')){
			return FALSE;
		//若已有設定值，則不再處理此區塊！
		}else if( is_array($this->_session->CONST->WEEK) ){
			return TRUE;
		}
